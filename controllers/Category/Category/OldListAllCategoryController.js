...
const OldListAllCategorySchema = require('../../../models/Category/Category/OldListAllCategorySchema');
const ApiResponseJson = require('api-response-json')

exports.on = (req, res, next) => {
    OldListAllCategorySchema.find()
...
exports.onId = (req, res, next) => {
    OldListAllCategorySchema.find({ _id: req.params.id })
        .exec(function (error, data) {
            if (error) {
                return next(error);
                // return res.status(400).json({
...
exports.created = (req, res, next) => {
    ...
    var data = new OldListAllCategorySchema({
        icon: req.body.icon,
...
exports.update = (req, res, next) => {
    const data_id = OldListAllCategorySchema.where({ _id: req.params.id });
...
exports.remove = (req, res, next) => {
    OldListAllCategorySchema.remove({ _id: req.params.id })
        .exec()
        .then(data => {
