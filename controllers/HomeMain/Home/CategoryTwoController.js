...
const CategoryTwoSchema = require('../../../models/HomeMain/Home/CategoryTwoSchema');
const ApiResponseJson = require('api-response-json')

exports.on = (req, res, next) => {
    CategoryTwoSchema.find()
...
exports.onId = (req, res, next) => {
    CategoryTwoSchema.find({ _id: req.params.id })
        .exec(function (error, data) {
            if (error) {
                return next(error);
                // return res.status(400).json({
...
exports.created = (req, res, next) => {
    ...
    var data = new CategoryTwoSchema({
        id: req.body.id,
        name: req.body.name,
...
exports.update = (req, res, next) => {
    const data_id = CategoryTwoSchema.where({ _id: req.params.id });
...
exports.remove = (req, res, next) => {
    CategoryTwoSchema.remove({ _id: req.params.id })
        .exec()
        .then(data => {
