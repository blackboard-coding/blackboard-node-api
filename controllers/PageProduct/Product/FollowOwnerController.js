...
const FollowOwnerSchema = require('../../../models/PageProduct/Product/FollowOwnerSchema');
const ApiResponseJson = require('api-response-json')

exports.on = (req, res, next) => {
    FollowOwnerSchema.find()
...
exports.onId = (req, res, next) => {
    FollowOwnerSchema.find({ _id: req.params.id })
        .exec(function (error, data) {
            if (error) {
                return next(error);
                // return res.status(400).json({
...
exports.created = (req, res, next) => {
    ...
    var data = new FollowOwnerSchema({
    })
...
exports.update = (req, res, next) => {
    const data_id = FollowOwnerSchema.where({ _id: req.params.id });
...
exports.remove = (req, res, next) => {
    FollowOwnerSchema.remove({ _id: req.params.id })
        .exec()
        .then(data => {
