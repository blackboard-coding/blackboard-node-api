...
const ProductInfoSchema = require('../../../models/PageProduct/Product/ProductInfoSchema');
const ApiResponseJson = require('api-response-json')

exports.on = (req, res, next) => {
    ProductInfoSchema.find()
...
exports.onId = (req, res, next) => {
    ProductInfoSchema.find({ _id: req.params.id })
        .exec(function (error, data) {
            if (error) {
                return next(error);
                // return res.status(400).json({
...
exports.created = (req, res, next) => {
    ...
    var data = new ProductInfoSchema({
        product: req.body.product,
        vdo_list: req.body.vdo_list,
...
exports.update = (req, res, next) => {
    const data_id = ProductInfoSchema.where({ _id: req.params.id });
...
exports.remove = (req, res, next) => {
    ProductInfoSchema.remove({ _id: req.params.id })
        .exec()
        .then(data => {
